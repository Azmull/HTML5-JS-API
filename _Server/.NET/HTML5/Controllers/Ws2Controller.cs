  ...
  public class Ws2Controller : ControllerBase
  {
    ...
    private async Task ProcessWebSocket(WebSocket webSocket)
    {
      ...
      while (!res.CloseStatus.HasValue)
      {
        ...
        if (receivedMsg != null)
        {
          receivedMsg.UserId = webSocket.GetHashCode();
          receivedMsg.ChatTime = DateTime.Now.ToString("hh:mm:ss");
    ...
    private void Broadcast(MsgObj msgObj)
    {
      ...
      {
        if (webSocket.State == WebSocketState.Open && webSocket.GetHashCode() != msgObj.UserId)
        {
